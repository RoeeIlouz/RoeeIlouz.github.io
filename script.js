...
document.addEventListener('DOMContentLoaded', () => {
  ...
  function applyLanguage(lang) {
    ...
    // Sidebar
    Object.entries(dict.sidebar).forEach(([id, text]) => {
      const el = document.getElementById(id);
      if (el) el.innerHTML = text;
    });

    // Nav
    Object.entries(dict.nav).forEach(([id, text]) => {
      const el = document.getElementById(id);
      if (el) el.textContent = text;
    });

    // About
    Object.entries(dict.about).forEach(([id, text]) => {
      const el = document.getElementById(id);
      if (el) el.innerHTML = text;
    });

    // Experience & Skills
    Object.entries(dict.experience).forEach(([id, text]) => {
      const el = document.getElementById(id);
      if (el) el.innerHTML = text;
    });
  }
  ...
  navButtons.forEach(btn => {
    btn.addEventListener('click', () => {
      ...
      // Scroll smoothly to top of main content on mobile screens
